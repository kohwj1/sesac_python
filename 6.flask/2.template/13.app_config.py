...
app.config['ALLOWED_FILE_EXT'] = {'png', 'jpg', 'jpeg', 'gif'}
app.config['MAX_CONTENT_LENGTH'] = 1 * 1024 * 1024 #서버에서 정의한 용량 제한 --> Flask가 알아서 해줌

os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True) #폴더 생성 (있어도 허용)
...
def upload_file():
    ...
    #app.config['MAX_CONTENT_LENGTH']를 정의했다면 아래 내용을 구현하지 않아도 서버에 정의된 용량 초과로 처리되어 413 상태값을 리턴함
...
#http status에 따른 에러 핸들링
@app.errorhandler(413)
def too_large(e):
    size_mb = app.config['MAX_CONTENT_LENGTH'] / (1024 * 1024)
    return f'업로드한 파일이 너무 큽니다. 최대 {size_mb}MB까지 첨부할 수 있습니다.'
